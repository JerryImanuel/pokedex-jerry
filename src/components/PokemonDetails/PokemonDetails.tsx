...
const PokemonDetails = ({ pokemon }: PokemonDetailsProps) => {
  const [areInFavorite, setAreInFavorite] = useState(
    verifyIfIsFavorited(pokemon.id)
  );
      ...
      <div className="pokemon-details-card">
        ...
        <div className="pokemon-details-favorite">
          ...
          <div
            ...
            id={`${COMPONENT_ID}__favorite`}
          >
            {areInFavorite ? (
              <MdCatchingPokemon size={30} color="#C22E28" />
            ) : (
              <MdOutlineCatchingPokemon size={30} color="#373E59" />
            )}
          </div>
        </div>
          ...
          <div>
            ...
            <div className="pokemon-details-abilities">
              ...
              {pokemon.abilities.map((value: any, index: number) => {
                return (
                  <p key={index} className="pokemon-details-abilities__item">
                    {value.ability.name}
                  </p>
                );
              })}
          ...
          <div className="pokemon-details-stats">
            {pokemon.stats.map((value: any) => {
              return (
                <div
                  key={pokemon.id}
                  className="pokemon-details-stats__item"
                  style={{ textTransform: "capitalize" }}
                >
                  {value.stat.name}
